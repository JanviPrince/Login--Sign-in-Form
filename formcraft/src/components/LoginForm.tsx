...
export function LoginForm({ onToggleForm, onForgotPassword }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    alert('Logged in successfully! (Simulator)');
  };
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between text-xs mt-1">
        <label className="flex items-center gap-2 text-slate-600 dark:text-slate-400 cursor-pointer group">
          <input type="checkbox" className="w-4 h-4 rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-indigo-600 focus:ring-indigo-500 transition-colors" />
          <span className="group-hover:text-indigo-500 transition-colors">Remember me</span>
        </label>
        <button 
          type="button" 
      ...
      <div className="flex items-center gap-4 my-4">
        ...
        <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest leading-none">Or continue with</span>
        <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800" />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <button type="button" className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all font-medium text-sm">
